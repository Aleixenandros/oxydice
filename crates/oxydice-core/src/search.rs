...
/// Rutas de los `.md` bajo `root` cuyo *frontmatter* incluye `tag` (sin
/// distinguir mayúsculas; admite `#tag`). Recorrido directo del FS, omitiendo
/// ocultos, coherente con `search` (el disco es la verdad, sin índice).
pub fn notes_with_tag(root: &Path, tag: &str) -> Vec<PathBuf> {
    let needle = tag.trim().trim_start_matches('#').trim().to_lowercase();
    let mut out = Vec::new();
    if needle.is_empty() {
        return out;
    }
    tag_walk(root, &needle, &mut out);
    out
}

fn tag_walk(dir: &Path, needle: &str, out: &mut Vec<PathBuf>) {
    let Ok(rd) = std::fs::read_dir(dir) else {
        return;
    };
    let mut entries: Vec<PathBuf> = rd
...
mod tests {
    ...
    #[test]
    fn notes_with_tag_matches_frontmatter() {
        ...
        std::fs::write(d.join("sub/b.md"), "---\ntags:\n  - rust\n---\n# B").unwrap();
        std::fs::write(d.join("c.md"), "# sin tags").unwrap();

        let mut got = notes_with_tag(&d, "#RUST");
        got.sort();
        let names: Vec<_> = got
            .iter()
        ...
        assert!(notes_with_tag(&d, "").is_empty());
